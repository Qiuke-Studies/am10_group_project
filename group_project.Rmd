...
data_all <- data %>% 
  group_by(year, month,month_name, season_name, descript,day, hour, day_of_week, category, pd_district, resolution, x, y) %>% 
  count(year)
...
model1 <- lm(n ~ 1, data= data_all)
msummary(model1)

model2 <- lm(n ~ day_of_week + unemployment + cpi + pop + poverty_rate + personal_income, data= data_all)
msummary(model2)

model3 <- lm(n ~ day_of_week + unemployment + cpi + pop + poverty_rate + personal_income + resolution + factor(hour) + pd_district, data= data_all)
msummary(model3)
vif(model3)


model4 <- lm(n ~ day_of_week + unemployment + poverty_rate + personal_income + resolution + hour_cat + pd_district + season_name + factor(year), data= data_all)
msummary(model4)
