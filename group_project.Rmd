...
diffs <- data_all %>% 
  ...
  mutate(cpi_diff = cpi - 6.0,
         ...
         min_diffs = sum(cpi_diff, unemployment_diff, poverty_rate_diff))

diffs <- diffs %>% 
  mutate(is_min = ifelse(min_diffs == min(diffs$min_diffs), 1, 0)) %>% 
  select(year, is_min)
...
ggplot(data = data_line) +
  geom_line(aes(x = month_name, y = n, group = factor(year), color = factor(is_min))) +
  ...
  scale_color_manual(values = c("#EBE8E5", "#001E62"))
